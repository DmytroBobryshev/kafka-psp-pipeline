...
/**
 * One record read back off {@code payments.payment-status-changed.v1.ledger.dlq} by
 * {@code domain.port.DlqReader}, ready for {@code application.ReplayDlqUseCase} to republish
 * byte-for-byte to {@code payments.payment-status-changed.v1} via
 * {@code domain.port.DlqRepublisher}.
 *
 * <p>Deliberately opaque: {@code value} is the raw Confluent-wire-format bytes exactly as read off
 * the DLQ partition (magic byte + schema id + Avro binary - {@code payments.payment-status-changed.v1}
 * has been Avro since M9 Phase 2), carried as {@code byte[]} rather than decoded back into the
 * generated {@code com.example.psp.common.events.avro.PaymentStatusChanged} type. Two reasons:
 *
 * <ol>
 ...
 *
 * @param key     the record's Kafka key - the {@code merchantId} string, preserved unchanged on
 *                replay so the republished record keeps its original partition assignment
 *                (ADR-0003: the ledger's single-writer-per-balance invariant depends on it).
