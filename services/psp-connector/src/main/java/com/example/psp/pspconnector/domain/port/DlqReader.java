 ...
 * {@code payments.payment-requested.v1.psp-connector.dlq}, for
 ...
 * models a one-shot, on-demand read, not a subscription.
 */
public interface DlqReader {

    /**
     * Reads and commits up to {@code maxRecords} records from the DLQ topic under this reader's
     * own dedicated consumer group ({@code psp-connector.dlq-replay.consumer-group}), so records
     * already replayed are never returned again by a later call. Returns fewer than
     * {@code maxRecords} if the DLQ currently holds fewer than that many unreplayed records, and an
